...
ggplot(data = spp_rcbd_layout, aes(x = row, 
                                   y = -col))+
  geom_tile(aes(fill = factor(nrate_kgha)), show.legend = F,
            color = "black")+
...
spp_rcbd_rand %>%
  # Rearranging columns  
  dplyr::select(blocks, wplot, krate_kgha, 
                splot, nrate_kgha,
                trt_name) %>%
